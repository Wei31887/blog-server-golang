...
func (blog *Blog) FindPreviousBlog() (*Blog, error) {
	resBlog := new(Blog)
	db := G.GLOBAL_DB.Where("id < ?", blog.Id).Order("id desc").First(resBlog)
	if db.Error != nil {
		return nil, db.Error
	}
	return resBlog, nil
...
func (blog *Blog) FindBlogWithTypeName() (*Blog, error) {
	...
	db := G.GLOBAL_DB.Table("blog").Select("*, blog_type.name as type_name").
					...
					Where("blog.id = ?", blog.Id).Order("blog_type.sort asc").Find(resBlog)
...
func (blog *Blog) FindList(page utils.Page) ([]*Blog, error) {
	...
	curDB := G.GLOBAL_DB.Table("blog").Select("*, blog_type.name as type_name").
				Joins("left join blog_type on blog.typeid = blog_type.id")
...
func (blog *Blog) UpdataClick() (error) {
	db := G.GLOBAL_DB.Model(blog).Where("id = ?", blog.Id).Update("click_hit", gorm.Expr("click_hit + 1"))
	return db.Error
}
...
func (blog *Blog) FindBlogComment() ([]Comment, error) {
	...
	db := G.GLOBAL_DB.Table("comment").Where("blog_id = ? and status = 1", blog.Id).
